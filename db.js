...
/* READ-ONLY: every order attempt for a user (any status). Used by the provenance backfill to recover the durable
   origin of a trade from the attempt's `payload` (which carries the original tradeType / strategyName / screenerKey
   set BEFORE broker submission) via order_tag / broker_order_id / clientRequestId. Newest first. */
async function getOrderAttemptsForUser(userId, limit = 5000) {
  if (USING_PG) { const r = await pool.query(`SELECT * FROM order_attempts WHERE user_id=$1 ORDER BY created_at DESC LIMIT $2`, [String(userId), Number(limit) || 5000]); return r.rows.map(_rowFromPg); }
  const d = readJSON(_attemptFile()); return Object.values(d).filter((x) => x && String(x.userId) === String(userId)).sort((a, b) => (b.createdAt || 0) - (a.createdAt || 0)).slice(0, Number(limit) || 5000);
}
...
/* Release ALL of a user's UNKNOWN idempotency records at once. Used only by the admin manual override
   (ops/clear-halt) after the owner has verified out-of-band that the broker is flat — the account-wide
   new-entry block keys on countUnknownIdempotency(), so a stuck 'unknown' row (e.g. a timed-out order the
   client can no longer reconcile because it lost the key) would otherwise block every new order forever.
   Only touches 'unknown' rows — never a succeeded/in_flight record. Returns the number released. */
async function releaseUnknownIdempotency(userId) {
  if (USING_PG) {
    const r = await pool.query(`DELETE FROM order_idempotency WHERE user_id=$1 AND status='unknown'`, [String(userId)]);
    return r.rowCount || 0;
  }
  const f = FILES.idem || (FILES.idem = path.join(__dirname, "order_idempotency.json")); const d = readJSON(f);
  let n = 0; for (const k of Object.keys(d)) { if (k.startsWith(`${userId}|`) && d[k] && d[k].status === "unknown") { delete d[k]; n++; } }
  if (n) writeJSON(f, d);
  return n;
}
